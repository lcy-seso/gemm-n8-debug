...
struct AllReduce {
    static_assert(threads == 1024 or threads == 512 or threads == 256 or
                  ...
                  threads == 16 or threads == 8 or threads == 4 or
                  threads == 2);
...
template <int threads, int Axis = 0, bool reverse = false>
struct CumSum2D {
    static_assert(threads == 1024 or threads == 512 or threads == 256 or
                  threads == 128 or threads == 64 or threads == 32);
    template <typename T, int SEG = 32>
    static TL_DEVICE T run(const T* __restrict__ src, T* __restrict__ dst,
                           int H, int W) {
...
#pragma unroll
                    for (int off = 1; off < SEG; off <<= 1) {
                        ...
                        if (lane < SEG - off) val += n;
                    }

                    val += carry;

                    if (real_col < W) dst[real_row * W + real_col] = val;

                    T segSum = (T)__shfl_sync(MASK, val, (T)0);
                    if (lane == 0) carry = segSum;
