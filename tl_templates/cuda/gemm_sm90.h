...
class GemmTensorOp {
  public:
    ...
    static CUTE_DEVICE void body_rs(A_type_raw* pA, B_type_raw* pB,
        ...
        // asm volatile("bar.sync %0, %1;" : : "r"(1), "r"(num_warp_m *
        // num_warp_n * 32));
        const int tid = threadIdx.x;
        Tensor sB = make_tensor(make_smem_ptr(reinterpret_cast<B_type*>(pB)),
                                SmemLayoutB{});
...
class GemmTensorOp {
  public:
    ...
    template <class... Args>
    static CUTE_DEVICE auto remove_swizzle(
        ComposedLayout<Args...> const& layout) {
    ...
    static CUTE_DEVICE void body(A_type_raw* pA, B_type_raw* pB,
        ...
        // when layout is KxN and n_warp is 1, there seem to be a bug, use this
        // as a workaround
        auto tCrA_view =
    ...
    static CUTE_DEVICE void body_rs(A_type_raw* pA, B_type_raw* pB,
        ...
        for (int k = 0; k < size<2>(tCrA); ++k) {
            if (k < size<2>(tCrA) - 1) {
                copy(tiled_copy_B, tCsB(_, _, k + 1),
                     tCrB_copy_view(_, _, k + 1));
            }
    ...
    static CUTE_DEVICE void body_sr(A_type_raw* pA, B_type_raw* pB,
        ...
        for (int k = 0; k < size<2>(tCrA); ++k) {
            if (k < size<2>(tCrA) - 1) {
                copy(tiled_copy_A, tCsA(_, _, k + 1),
...
TL_DEVICE void gemm_ss(A_type* pA, B_type* pB, C_type* accum) {
    if constexpr (use_wgmma) {
        using MMA =
            cute::tl_wgmma::GemmTensorOp<M, N, K, num_warp_m, num_warp_n,
                                         trans_A, trans_B, clear_accum, A_type,
                                         B_type, C_type>;
...
TL_DEVICE void warp_scheduler_barrier_sync() {
    cutlass::arch::NamedBarrier::sync(
        NumMmaThreads, cutlass::canonical_warp_group_idx() /*id*/);
}
